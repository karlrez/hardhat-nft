...
!developmentChains.includes(network.name)
    ...
    : describe("Nft Marketplace Unit Tests", function () {
          ...
          const TOKEN_ID = 0
          const tokenURI = EXAMPLE_METADATA.boy.image
          let deployer: Signer
          let user: Signer
          ...
          beforeEach(async () => {
              ...
              await basicNft.safeMint(deployer.getAddress(), tokenURI)
          ...
          describe("listItem", function () {
              ...
              it("exclusively items that haven't been listed", async function () {
                  ...
                  // this works if our error is a String, but we are using a custom error

                  //   await expect(
                  //       nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
                  //   ).to.be.revertedWith(error)

                  // Checking name of custom error is correct
                  await expect(
                      nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
